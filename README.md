...
### 3.6 技术链路：谁在判断、谁在查（中间件视角）

把系统想象成一组"中间件"串起来，一次提问会经过这些环节：

| 环节 | 谁在工作 | 干什么 |
|---|---|---|
| 改写 / 判断来意 / 抽约束 / 选工具 / 写回答 | **大模型（DeepSeek）** | 理解语言、做判断 |
| 内容检索的"语义相似度匹配" | **向量模型（bge-m3）+ 内存向量库** | 问"阅读课讲什么"→ 找语义最像的教材段落 |
| 真正执行查询 | **后端本地工具（7 个）→ 预写 SQL → SQLite** | 查课程 / 价格 / 套餐 / 时长 |
| 记录需求画像 | **后端 → SQLite** | 每轮约束累积落库 |

> 说明：这里的"工具"是**后端本地函数**（Spring AI FunctionToolCallback），**不是 MCP 工具**。工具内部只执行**人工预写的参数化 SQL**（查 SQLite）或**向量检索**，大模型永远不直接碰数据库。
...
sequenceDiagram
    participant U as 用户
    participant F as 页面
    participant B as 后端大脑
    participant L as 大模型 DeepSeek
    participant T as 本地工具
    participant S as SQLite 数据库
    participant V as 向量库 bge-m3

    U->>F: 输入一句话
    F->>B: /api/chat（SSE 长连接）
    B->>L: ① 查询改写（补全指代）
    L-->>B: 完整问题
    B->>L: ② 判断来意 + ③ 抽约束
    L-->>B: 意图 + 新约束
    B->>S: 画像持久化（约束累积）
    loop ④ 工具循环（可多轮）
        B->>L: 把上下文交给大模型，问"需要查什么"
        L-->>B: 选择工具 + 参数（tool_calls）
        alt 查课程/价格/套餐/时长
            B->>T: 执行本地工具
            T->>S: 预写参数化 SQL
            S-->>T: 真实数据
...
| 用户说的情况 | 走的分支 | 不走的 |
|---|---|---|
| 闲聊（"你好"） | 只走大模型，直接聊 | 不查任何数据 |
| 问价格 / 课程 / 套餐 / 时长 | 大模型判断 → 本地工具 → SQLite | 不走向量检索 |
| 问"课里教什么" | 大模型判断 → 向量检索（语义匹配） | 不走 SQL 筛选 |
| 库里没有的（围棋课 / 退费政策） | 工具查到空 → 大模型如实说"没有" | 不编造、不强推 |
...
一位零基础用户想学英语，和 AI 聊了 6 轮。6 轮的整体调用链路：

```mermaid
flowchart TB
    A["轮1 我想学英语<br/>大模型判断:来意=课程推荐<br/>本地工具:课程检索 → SQLite 全表"] --> B
    B["轮2 我基础差<br/>大模型抽取新约束:零基础<br/>本地工具:课程检索 → SQLite 过滤"] --> C
    C["轮3 那大概多少钱<br/>大模型改写指代 + 判来意=价格<br/>本地工具:价格查询 → SQLite 定价表"] --> D
    D["轮4 一个月内学完<br/>大模型抽取新约束:≤30天<br/>本地工具:时长检查 → SQLite"] --> E
    E["轮5 有套餐划算吗<br/>大模型判来意=套餐<br/>本地工具:套餐匹配 → SQLite 套餐表"] --> F
    F["轮6 就推荐雅思的<br/>大模型判来意=收口<br/>本地工具:收口推荐 + 下单 → 生成订单号"]
```

**第 1 轮**

- 用户说：`我想学英语，有什么课？`
- 技术链路：大模型判断来意=课程推荐 → 本地工具「课程检索」→ 预写 SQL 查 SQLite 课程表（全表）
...
- 用户说：`我基础比较差，想从零开始`
- 技术链路：大模型抽取新约束 {水平:零基础} → 本地工具「课程检索」→ 预写 SQL 查 SQLite，按约束过滤
- 查到什么：候选从 10 门**收窄到入门课**（自然拼读 30 天 / 小学英语基础 60 天 / 雅思基础班 45 天等）
- 为什么走这个分支：用户给了新约束"零基础"，所以只推荐入门级课程（本轮走 SQL 过滤，不走向量检索）
- 回答：列出入门课，追问"您想多长时间内学完？"

**第 3 轮**

- 用户说：`那大概要多少钱？`
- 技术链路：大模型先**改写**（"那"→ 上一轮推荐的入门课），再判来意=价格咨询 → 本地工具「价格查询」→ 预写 SQL 查 SQLite 定价表
- 查到什么：各门课单价（自然拼读 ¥999 / 小学英语基础 ¥1999 / 雅思基础班 ¥1999……）
- 为什么走这个分支：这句话不带新约束、明显在问钱，所以走价格分支（不走向量检索）
- 回答：直接报出价格清单（**没有**反问"你想学什么"——证明改写生效）

**第 4 轮**

- 用户说：`我想一个月内能学完的`
- 技术链路：大模型抽取新约束 {时间:≤30 天} → 本地工具「时长检查」→ 预写 SQL 查 SQLite，筛时长≤30 天
...
- 用户说：`有套餐更划算吗？`
- 技术链路：大模型判来意=套餐咨询 → 本地工具「套餐匹配」→ 预写 SQL 查 SQLite 套餐表，并计算"单买累加 vs 套餐价"
...
- 用户说：`就推荐雅思的那个吧`
- 技术链路：大模型改写"雅思的那个"→ 判来意=收口 → 本地工具「收口推荐」（综合 6 轮画像评分）→ 本地工具「下单」→ 生成订单号
- 查到什么：最终方案（课程+价格+时长）→ 订单号 CO-20260802-001
- 回答：推荐方案卡片 + 订单号 + "请在 15 分钟内完成支付"

> 这条例子完整展示了价值闭环：**约束逐轮累积 → 候选从 10 门收到 3 门 → 报真实价格 → 算套餐省钱 → 下单**；全程"理解/判断"由大模型负责，"查数据"由本地工具 + SQLite 负责。
...
- 技术链路：大模型判来意=学习规划，抽取约束 {目标课程:雅思基础班, 时间:≤20 天} → 本地工具「时长检查」→ 预写 SQL 查 SQLite 课程表，查"雅思基础班"时长
- 查到什么：雅思基础班需 **45 天** > 用户只有 20 天 → **无直接命中（冲突）**
- 为什么走这个分支：约束与课程时长矛盾，工具返回"无匹配"，触发冲突兜底逻辑（本轮 SQL 查到结果但判定不满足约束，不走向量检索）
